// api/_lib/brevo.js — sends the access-code email via Brevo's transactional
// email API. Requires BREVO_API_KEY. Optionally BREVO_SENDER_EMAIL / BREVO_SENDER_NAME.
'use strict';
    ...
    return retryAsync(
        async (attempt, signal) => {
            ...
            if (!res.ok) {
                ...
                const err = new Error(`Brevo send failed: ${res.status} ${errText}`);
                err.status = res.status;
                // A 400 usually means a malformed/invalid recipient address — that
                // will never succeed on retry, so don't waste attempts on it.
                if (res.status === 400) err.status = 400;
                throw err;
            }
        ...
        },
        {
            ...
            baseDelayMs: 600,
            timeoutMs: 8000,
            label: 'brevo.sendAccessCodeEmail',
            shouldRetry: (err) => err.status !== 400 && (!err.status || err.status === 429 || err.status >= 500),
        }
    );
}
