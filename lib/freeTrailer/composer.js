/**
 * FREE TRAILER — browser-side generator.
 * Canvas (2D) + MediaRecorder + Web Audio API only. No FFmpeg. No Luma. No server round-trip.
 * Output: WebM Blob (video/webm;codecs=vp9,opus or vp8/opus fallback).
 *
 * This file is intentionally self-contained and does not import Supabase or any
 * Premium-pipeline code. It only reads what is passed in.
 */

// Minimal theme accent/background lookup (mirrors app.html THEMES/FRIENDSHIP_THEMES a1/bg,
// duplicated here on purpose so this module has zero dependency on app.html internals).
const THEME_COLORS = {
...
/** 6–15 photos → 10–20s, more photos extend automatically, floor/ceiling applied. */
...
function drawCoverImage(ctx, img, x, y, w, h, panX, panY, scale) {
    // center-crop to fill w×h, then apply extra Ken-Burns scale + pan around center
 ...
 * @param {string|null} input.startDate  (pages.start_date, 'YYYY-MM-DD')
 * @param {string} input.theme           (pages.theme id, may be null/undefined -> default)
 * @param {string[]} input.photoUrls     (public URLs, already uploaded)
 * @param {string|null} input.musicUrl   (public URL, may be null -> silent video)
 * @param {'love'|'friendship'} [input.edition]
 * @param {'en'|'ar'} [input.lang]
 * @param {(p:{phase:'preparing'|'rendering'|'finalizing', pct:number})=>void} [onProgress]
...
export async function generateFreeTrailer(input, onProgress) {
    ...
    // 1. Load photos (skip individual failures, but keep going)
    const images = [];
    for (let i = 0; i < photoUrls.length; i++) {
        try {
            images.push(await loadImage(photoUrls[i]));
        } catch (e) {
            console.error('[free-trailer] photo load failed, skipping:', photoUrls[i], e);
        }
    ...
    // 2. Load music (optional — video still generates silently if this fails)
    ...
    if (audioBuffer && audioCtx) {
        ...
        srcNode.loop = audioBuffer.duration < totalDuration; // loop only if shorter than video, never leave silence
    ...
    // Target ~2 Mbps @ 720x1280/30fps to keep the file reasonably small.
    const recorder = new MediaRecorder(combined, { mimeType, videoBitsPerSecond: 2_000_000, audioBitsPerSecond: 128_000 });
    const chunks = [];
    recorder.ondataavailable = (e) => { if (e.data && e.data.size > 0) chunks.push(e.data); };

    // Low-power detection: start conservative on devices that look weak, and also
    // adapt at runtime if actual frame pacing turns out to be slow — either way,
    // blur transitions (canvas filter blur is expensive) and particles are cut,
    // never the core Ken Burns / crossfade / typography, to keep rendering smooth.
    ...
    function updateLowPowerFromPacing(dt) {
        ...
        if (frameTimes.length === 24) {
            ...
            if (avg > 1 / 20) { // sustained under ~20fps -> degrade gracefully
                lowPower = true;
                console.error('[free-trailer] slow device detected, disabling blur transitions + particles');
            }
        }
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        function frame(now) {
            ...
            // background theme wash first (visible during any transparent edges)
        ...
        try {
            ...
            if (srcNode && audioBuffer && audioBuffer.duration > totalDuration - 1) {
                // fade out during the last second if music outlasts the video
